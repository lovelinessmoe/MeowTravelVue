				...
				<section-title>
					<div style="display: flex;align-items: center;justify-content: space-between;">
						...
						<div style="font-size: 13px;">
							<span
								style="color: #9c9c9c;margin-right: 20px;">{{ this.$moment(comment.createTime).format('YYYY-MM-DD') }}</span>
							<span v-if="comment.pId === '0'" style="cursor: pointer;"
							      @click.stop="showCommentEditor=true">回复</span>
						</div>
					</div>
				</section-title>
			</div>
		...
		<div class="comment-body">
			<div class="content-text">
				...
				<mavon-editor
					v-model="content"
					:boxShadow="false"
					:editable="false" :ishljs="true"
					:subfield="false" :toolbarsFlag="false"
					defaultOpen="preview" style="z-index: 0;"
				/>
			</div>
			<div v-if="showCommentEditor" @click.stop="">
				<mavon-editor v-model="this.replyComment.content"/>
				<el-button circle icon="Check" style="float: right; position: relative;top: -32px;z-index: 999999;"
				           type="success"
				           @click="submitReply"/>
